...
async function main() {
  ...
  const groups = await coll
    .aggregate([
      {
        $group: {
          _id: { dateKey: '$dateKey', home: '$homeLabel', away: '$awayLabel' },
          docs: { $push: '$$ROOT' },
          count: { $sum: 1 },
        },
      },
  ...
  console.log(`Found ${groups.length} core groups (date+home+away) with >1 doc.`);
  let removed = 0;
  let mergedCount = 0;

  for (const g of groups) {
    const { dateKey, home, away } = g._id;
    const docs = g.docs;
    ...
    merged.groupLabel = merged.groupLabel ?? null;
    merged.timeLabel = merged.timeLabel ?? null;
    ...
    const canonicalId = makeDeterministicId([merged.groupLabel ?? '', dateKey, merged.homeLabel, merged.awayLabel].join('|').toLowerCase());
    merged.id = canonicalId;

    // delete old docs and insert merged
  ...
  console.log(`Merged ${mergedCount} groups, removed ${removed} extra documents.`);
