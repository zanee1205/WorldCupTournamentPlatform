...
export function MatchListPage({ matches, onOpenMatch }: MatchListPageProps) {
    ...
    const columns: TableColumnsType<TournamentMatch> = [
        {
            ...
            width: 80,
            render: (value, match) => (
                <Button type="link" onClick={() => onOpenMatch(match)}>
                    #{value}
                </Button>
            ),
        },
    ...
    const filteredMatches = useMemo(() => {
        ...
        // ensure opening match (earliest overall) appears first when present in the filtered list
